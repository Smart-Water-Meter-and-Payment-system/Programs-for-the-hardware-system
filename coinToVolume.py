...
def recordFetchingDetails():
	...
	registerCustomer(phoneNumber, roundedAmountLeft)
...
def getTempCoinAmount(amount):
	amountIn = amount
	if amount == 0:
		print("The amount is not valid")
	else:
		chargeAmount = getWaterChargeRate()
		volume = getWaterChargeVolume()
		volumeToBeDispensed = (amount * volume)/chargeAmount
		print("Volume to be dispensed: ", volumeToBeDispensed)
...
if __name__ == '__main__':
	...
	while True:
		try:
			start_counter=1
			inputEntered = input("Enter an amount:")
			setup(33,16,37)
			getCoinAmount(inputEntered)
